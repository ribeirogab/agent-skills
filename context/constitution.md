...
- **Symlink discipline**: `.claude/skills/harness` is a symlink back to `skills/harness/` so the host skill is dogfooded on the repo itself. **Never** re-introduce a blanket symlink `.claude/skills → skills/`. Only individual skills inside `skills/` may be exposed via per-skill symlinks. Anything else copied or generated under `.claude/skills/` must be a real directory committed to the repo, not a symlink.
