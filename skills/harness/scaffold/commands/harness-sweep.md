...
for f in context/learnings/*.md; do
  base=$(basename "$f" .md)
  hits=$(grep -rln --include='*.md' --exclude-dir=learnings "\[\[$base" context/ 2>/dev/null | wc -l)
...
grep -rohE '\[\[[^]|#]+' context/ \
  ...
  | while read -r target; do
      target_path="${target%/}"
      [ -f "context/$target_path.md" ] || [ -f "context/$target_path/index.md" ] \
        || [ -d "context/$target_path" ] \
        || echo "BROKEN: [[$target]]"
    done
```
...
for moc in context/_index/*.md; do
  grep -oE '\[\[[^]|#]+' "$moc" | sed 's/\[\[//' | while read -r t; do
    [ -f "context/$t.md" ] || [ -d "context/$t" ] \
      || echo "STALE in $(basename "$moc"): [[$t]]"
  done
done
```
...
```bash
for spec_dir in context/specs/[0-9]*-*/; do
...
```bash
for moc in context/_index/*.md; do
  awk '/^## /{ if (h && !c) print FILENAME": "h; h=$0; c=0; next } /^- |\[\[/{ c++ } END{ if (h && !c) print FILENAME": "h }' "$moc"
done
```
