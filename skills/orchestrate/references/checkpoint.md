...
2. Treat the supplied path as the user's request to lift this checkpoint pause. Preserve the approved plan, destination, tracking policy, and delegation policy. No extra resume phrase or start question is needed when start authorization remains valid. This request does not approve a pending plan, supply missing initial start authorization, override cancellation or another unresolved user hold, or expand permissions. Resolve those cases before affected work proceeds.
...
4. Confirm that no previous orchestrator or executor is still driving the same work before dispatching replacements. Query running or uncertain operations before retrying them, especially deployments and tracking writes. Verify required hard gates in the main session. Preserve user edits and completed actions; record executor replacements under the approved delegation policy. If exclusive control or an operation's result cannot be established, keep the affected work paused rather than duplicating it.
5. Apply the persistence rules before writing. Record resumption, the current session reference when available, and the next verified action in `ORCHESTRATION.md`, then mark the checkpoint `resumed` with the date. The checkpoint remains a snapshot, not a live progress ledger. Send a concise update and continue at **5. Execute until the objective is met** in `SKILL.md`; do not end the turn solely to ask whether to resume. If nothing can proceed, report the specific blocker and required action instead.

**Ready when:** the correct worktree and approved delivery are verified, outstanding operations are reconciled or safely isolated, no competing executor owns the next work, and the next authorized action is known.
