...
6. Report the stopping point, ongoing operations, and checkpoint path in a short message; the saved files carry the details. When ready, give `/orchestrate <absolute-path-to-CHECKPOINT.md>` as the complete resume instruction and identify the worktree to use. **End the turn and wait.** Do not continue implementation or dispatch work after saving the checkpoint.
